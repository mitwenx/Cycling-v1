...
export default function Dashboard() {
    const = useState({ lat: 0, lon: 0, speed_kph: 0, power: 0, dist_km: 0, time: 0, bearing: 0, status: 'Init', recording: false, paused: false });
    const = useState<[]>([]);
    ...
    useEffect(() => {
        const connect = () => {
            ...
            ws.current.onmessage = (e) => {
                ...
                setStats(data);
                if (data.lat && data.lon) setPath(prev =>]);
            };
    ...
    return (
        <div className="relative h- overflow-hidden flex flex-col">
            {/* Map Area */}
            <div className="flex-1 relative bg-gray-900 rounded-b-3xl overflow-hidden shadow-xl z-0">
                <MapContainer center={} zoom={15} zoomControl={false} style={{ height: '100%', width: '100%', position: 'absolute' }}>
                    <TileLayer url="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png" />
                    <Polyline positions={path} color="#a8c7fa" weight={5} />
                    {stats.lat !== 0 && <Marker position={} icon={customIcon} />}
                    <MapUpdater center={} bearing={stats.bearing} compassMode={compassMode} />
                </MapContainer>

                <button onClick={() => setCompassMode(!compassMode)} className={`absolute top-4 right-4 z- p-3 rounded-full shadow-lg ${compassMode ? 'bg-primary text-on-primary' : 'bg-surface text-gray-200'}`}>
                ...
                <div className="bg-surface-container/95 backdrop-blur-md p-5 rounded-3xl shadow-2xl border border-gray-700">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="flex flex-col">
                            <span className="text- text-gray-400 uppercase font-bold">Power</span>
                            <span className="text-xl font-bold text-primary">{stats.power} W</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text- text-gray-400 uppercase font-bold">Distance</span>
                            <span className="text-xl font-bold">{stats.dist_km.toFixed(2)} km</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text- text-gray-400 uppercase font-bold">Time</span>
